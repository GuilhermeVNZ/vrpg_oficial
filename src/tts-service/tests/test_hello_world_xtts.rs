//! Teste simples para verificar se o XTTS gera áudio inteligível para "Hello World"

use std::io::Write;
use tts_service::xtts::{SynthesisRequest, XttsModel};

#[tokio::test]
async fn test_hello_world_xtts() {
    ...
    if !coqui_available {
        ...
        println!("   Or run: cargo test --test test_hello_world_xtts_stub");
        return;
    }
    ...
    // Sintetizar
    println!("🎤 Synthesizing 'Hello World' with XTTS...");
    let audio = model
        .synthesize(&request)
        .await
    ...
    let wav_path = "test_hello_world_xtts.wav";
